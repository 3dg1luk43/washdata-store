...
- **One search bar for the whole library**: The landing box searched brand names only, so a model number like `SN636X00PD` returned nothing. It now searches **brands, appliances, programs and reference cycles** in one pass, grouped by kind, and every row navigates into the normal browse views (a cycle row opens its trace with the full breadcrumb rebuilt, and a crumb back to the results).
  ...
  Matching is **substring and multi-token**, not prefix: `wm14` finds `iQ300 WM14N292` (49 matches where the old box found none), and `siemens dishwasher` matches across the brand, the model and the appliance type at once. Firestore has no full-text index and cannot do this, so the catalog is published as a static snapshot - `search-index.json`, 1,580 rows, 63 KB gzipped - that the browser matches entirely client-side for **zero** Firestore reads. Reference cycles stay a live prefix query: they were half the index's bytes (a third of it incompressible document ids) and a cycle carries no text of its own beyond the program name its profile row already has.

  **New contributions do not wait for the next rebuild.** The snapshot records when it was taken, and the client separately fetches everything created after that instant, then runs it through the same matcher - so a brand, appliance or program added a minute ago is searchable a minute later. That delta is fetched once per cache window rather than per keystroke, and a contribution drops the cache, so your own upload is searchable immediately. The snapshot is rebuilt and republished by the deploy workflow on every push and once a day; the cadence only decides how large the delta is, never whether something can be found. If the index file fails to load, search falls back to the old live prefix queries and says so in the results header.

  Two bounded staleness notes: the snapshot cannot see a **removal** or a pending-to-approved **promotion**, so a removed row can linger until the next rebuild (it resolves to a clean "not found" when opened) and a promoted row can show a stale Pending badge on the search card only. **Requires `firebase deploy --only firestore:indexes`** for seven new indexes: `(status, model_lc)` and `(status, brand_lc)` on devices, `(status, program_lc)` on profiles and cycles, and `(status, createdAt)` on brands, devices and profiles for the delta.
...
- **A brand's dishwashers were missing from its model list**: Siemens showed 2 of its 31 dishwashers, Bosch 0 of 37, Miele 1 of 30. Opening a brand fetched at most 60 devices and filtered by appliance type *in the browser*, and which 60 arrived was not arbitrary: nearly every device sits on `favoriteCount: 0`, so Firestore fell back to its implicit `__name__` tiebreak in the last sort key's direction (descending). Device ids are `{type}__{brand}__{model}`, so descending ranks `washer_dryer` > `washer` > `dryer` > `dishwasher` - dishwashers sorted last and were the first thing the cap dropped. Picking "Dishwasher" from the filter could not help, because it only re-read the page already in hand.

  The type filter is now part of the query, its options come from the full appliance-type list rather than from whatever the page returned, the merged page is no longer truncated, and the brand view has a working **Load more** (`__name__` is now named explicitly, because a cursor on `favoriteCount` alone skips the whole tie group it lands in). All 31 Siemens dishwashers and all 110 of its models are now reachable.
